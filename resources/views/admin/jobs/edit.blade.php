    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.jobs.update", [$job->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('job_nr') ? 'is-invalid' : '' }}" type="number" name="job_nr" id="job_nr" value="{{ old('job_nr', $job->job_nr) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jobtitle') ? 'is-invalid' : '' }}" type="text" name="jobtitle" id="jobtitle" value="{{ old('jobtitle', $job->jobtitle) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('questions') ? 'is-invalid' : '' }}" name="questions[]" id="questions" multiple>
                    @foreach($questions as $id => $questions)
                        <option value="{{ $id }}" {{ (in_array($id, old('questions', [])) || $job->questions->contains($id)) ? 'selected' : '' }}>{{ $questions }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('anmerkungen') ? 'is-invalid' : '' }}" name="anmerkungen" id="anmerkungen">{{ old('anmerkungen', $job->anmerkungen) }}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('milestone_1') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="milestone_1" id="milestone_1" value="1" {{ $job->milestone_1 || old('milestone_1', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('milestone_2') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="milestone_2" id="milestone_2" value="1" {{ $job->milestone_2 || old('milestone_2', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('milestone_3') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="milestone_3" id="milestone_3" value="1" {{ $job->milestone_3 || old('milestone_3', 0) === 1 ? 'checked' : '' }}>
